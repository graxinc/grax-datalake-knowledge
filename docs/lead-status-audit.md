...
lead_domains AS (
    ...
    WHERE l.email IS NOT NULL 
      ...
      -- Active lead criteria (MCL/MQL)
      AND (l.status IN ('Open', 'Working'))
),
-- Extract domains from customer accounts (Type-based only)
account_domains AS (
    SELECT DISTINCT
        a.id as account_id,
...
-- Final safety check: Ensure no prospects matched
...
SELECT 
    ...
    with_website,
    with_contacts,
    -- Flag which types are included in customer matching
    CASE 
...
- Improves lead qualification accuracy using Account Type governance
- Enables better territory and quota planning

### Data Governance

- Establishes Account Type as single source of truth for customer classification
